...
export class SolicitudComponent implements OnInit {
    ...
    getSolicitudMatricula() {
        this.service.get('matriculas/estudiantes/solicitud_matricula?identificacion=' + this.buscador + '&periodo_lectivo_id='
            + this.periodoLectivoSeleccionado.id).subscribe(
            response2 => {
    ...
    getAsignaturas() {
        ...
        this.service.get('matriculas/estudiantes/solicitud_matricula?identificacion=' + this.buscador + '&periodo_lectivo_id='
            ...
            error => {
                this.spinner.hide();
                this.flagSolicitud = false;
                if (error.error.errorInfo[0] === '404') {
                    swal.fire(this.messages['error404']);
                } else {
                    swal.fire(this.messages['error500']);
                }
    ...
    filter(event) {
        if (event === null && this.buscador.length !== 0) {
            this.getAsignaturas();
        } else {
